...
proc ::tcl::clock::scan { args } {
	...
	switch -exact -- $flag {
		...
		return -code error \
		    ...
		    "無效選項 \"$flag\",\
                     必須是 -base, -format, -gmt, -locale 或 -timezone"
	    }
	}
    }
	...
	return -code error \
	    ...
	    "不能在一個呼叫中同時使用 -gmt 和 -timezone"
    }
    if { [catch { expr { wide($base) } } result] } {
	return -code error "期望整數值但得到 \"$base\""
    }
    if { ![string is boolean -strict $gmt] } {
	return -code error "期望布爾值但得到 \"$gmt\""
    } elseif { $gmt } {
	set timezone :GMT
    }
...
proc ::tcl::clock::FreeScan { string base timezone locale } {
	...
	return -code error \
	    "無法轉換 date-time 字串 \"$string\": $message"
    }
...
proc ::tcl::clock::SetupTimeZone { timezone } {
	...
	if { $timezone eq {:localtime} } {
		...
		dict unset opts -errorinfo
		return -options $opts " $timezone 時區找不到"
	    }
...
proc ::tcl::clock::ReadZoneinfoFile {fileName fname} {
	...
	return -code error "$fileName 找不到"
    }

    if { [file size $fname] > 262144 } {
	return -code error "$fileName 太大"
    }
    ...
    set iformat I
    if { $magic != {TZif} } {
	return -code error "$fileName 檔案未包含時區資訊"
    }
    if { $nType > 255 } {
	return -code error "$fileName 檔案包含太多時間格式"
    }
    # Accept only Posix-style zoneinfo.  Sorry, 'leaps' bigots.
    if { $nLeap != 0 } {
	return -code error "$fileName 檔案包含閏秒資訊"
    }
	...
	binary scan $d @${seek}a4a1x15IIIIII \
	    magic version nIsGMT nIsStd nLeap nTime nType nChar
	if {$magic ne {TZif}} {
	    return -code error "尋找地址 $seek 計算錯誤，魔術值 = $magic"
	}
	set iformat W
	set ilen 8
	incr seek 44
	...
	if { $t < $lastTime } {
	    return -code error "$fileName 檔案中的時間順序錯誤"
	}
	set lastTime $t
...
proc ::tcl::clock::add { clockval args } {
	...
	return -code error \
	    -errorcode [list CLOCK gmtWithTimezone] \
	    "一個呼叫中不能同時使用 -gmt 和 -timezone"
    }
    if { [catch { expr { wide($clockval) } } result] } {
	return -code error "期望是整數值但是得到 \"$clockval\""
    }
    if { ![string is boolean -strict $gmt] } {
	return -code error "期望是布林值但是得到 \"$gmt\""
    } elseif { $gmt } {
	set timezone :GMT
    }
	...
	foreach { quantity unit } $offsets {
		...
		default {
			...
			"未知時間單位 \"$unit\"，必須是 \
                        years, months, weeks, days, hours, minutes 或 seconds"
		}
	    }
	}
	return $clockval
