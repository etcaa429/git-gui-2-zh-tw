...
proc ::tcl::HistAdd {event {exec {}}} {
	...
	return -code error "無效參數 \"$exec\"：必須是 \"exec\""
    }
...
proc ::tcl::HistKeep {{count {}}} {
	...
	return -code error "非法保留計數 \"$count\""
    }
...
proc ::tcl::HistIndex {event} {
    ...
    if {$i <= $history(oldest)} {
	return -code error "事件 \"$event\" 距離當前時間太久遠"
    }
    if {$i > $history(nextid)} {
	return -code error "事件 \"$event\" 尚未發生"
    }
    return $i
}

