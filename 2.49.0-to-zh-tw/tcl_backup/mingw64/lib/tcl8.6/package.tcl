...
proc pkg_mkIndex {args} {
	...
	switch -glob -- $flag {
	    ...
	    -* {
		return -code error "未知標誌 $flag：正確格式應為\n$usage"
	    }
	    default {
		# done with the flags
		break
	...
	if {$loadPat ne ""} {
	    if {$doVerbose} {
		tclLog "當前已載入的包: '[info loaded]'"
		tclLog "嘗試載入所有匹配 $loadPat 的包"
	    }
	    if {![llength [info loaded]]} {
		tclLog "警告: 當前沒有載入任何包，因此"
		tclLog "不可能有包匹配 '$loadPat'"
	    }
	}
	foreach pkg [info loaded] {
	    ...
	    if {$doVerbose} {
		tclLog "包 [lindex $pkg 1] 匹配 '$loadPat'"
	    }
	    try {
		load [lindex $pkg 0] [lindex $pkg 1] $c
	    } on error err {
		if {$doVerbose} {
		    tclLog "警告: 載入 [lindex $pkg 0] [lindex $pkg 1]\n失敗，錯誤資訊: $err"
		}
	    } on ok {} {
		if {$doVerbose} {
		    tclLog "已載入 [lindex $pkg 0] [lindex $pkg 1]"
		}
	    }
	...
	try {
	    ...
	    if {$doVerbose} {
		tclLog "警告: 在 $what $file 時發生錯誤: $msg"
	    }
	} on ok {} {
	    set what [$c eval set ::tcl::debug]
	    if {$doVerbose} {
		tclLog "$what $file 操作成功"
	    }
...
proc ::tcl::Pkg::Create {args} {
    ...
    set err(wrongNumArgs) "參數數量錯誤：應為 \"$err(usage)\""
    set err(valueMissing) "\"%s\" 的值缺失：應為 \"$err(usage)\""
    set err(unknownOpt)   "未知選項 \"%s\"：應為 \"$err(usage)\""
    set err(noLoadOrSource) "必須至少指定 -load 和 -source 選項中的一個"
