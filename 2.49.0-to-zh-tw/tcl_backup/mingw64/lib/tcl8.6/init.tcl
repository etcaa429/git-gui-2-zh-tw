...
if {[info commands package] == ""} {
    error "版本不匹配：庫指令碼要求 Tcl 版本為 7.5b1 或更高，但當前載入的版本是 [info patchlevel]"
}
...
proc unknown args {
	...
	if {[info exists UnknownPending($name)]} {
	    return -code error "命令 \"$name\" 的 \"unknown\" 處理中存在自引用遞迴"
	}
	...
	if {[llength $cmds]} {
	    return -code error "命令名 \"$name\" 存在歧義: [lsort $cmds]"
	}
    }
...
proc auto_load_index {} {
	...
	if {$issafe} {
		...
		if {$id eq "# Tcl autoload index file, version 2.0"} {
		    ...
		    error "[file join $dir tclIndex] 不是有效的 Tcl 索引檔案"
		}
	    } msg opts]
	    if {$f ne ""} {
		close $f
...
proc tcl::CopyDirectory {action src dest} {
	...
	if {$nsrc in [file volumes]} {
	    return -code error "執行 $action 操作時出錯：無法對 \"$src\" 執行此操作至\
      \"$dest\"，因為嘗試重新命名卷或移動目錄到自身"
	}
    }
    if {[file exists $dest]} {
	if {$nsrc eq $ndest} {
