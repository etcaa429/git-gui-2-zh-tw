...
constructor pick {} {
	...
	if {$top eq {.}} {
		...
		if {[is_MacOSX]} {
			...
			$w.mbar.apple add command \
				-label [mc "關於 %s" [appname]] \
				-command do_about
			$w.mbar.apple add command \
				-label [mc "顯示 SSH 金鑰"] \
				-command do_ssh_key
		} else {
			$w.mbar add cascade -label [mc 幫助] -menu $w.mbar.help
			menu $w.mbar.help
	...
	$opts insert end [mc "創建新倉庫"] link_new
	$opts insert end "\n"
	if {$m_repo ne {}} {
		$m_repo add command \
	...
	$opts insert end [mc "克隆已有倉庫"] link_clone
	$opts insert end "\n"
	if {$m_repo ne {}} {
	...
	$opts insert end [mc "開啟已有倉庫"] link_open
	$opts insert end "\n"
	if {$m_repo ne {}} {
		$m_repo add command \
...
method _do_new {} {
	...
	${NS}::label $w_body.where.l -text [mc "目錄："]
...
proc _new_ok {p} {
	if {[file isdirectory $p]} {
		if {[_is_git [file join $p .git]]} {
			error_popup [mc " %s 目錄已存在。" $p]
			return 0
		}
	} elseif {[file exists $p]} {
		error_popup [mc " %s 檔案已存在。" $p]
		return 0
	}
	return 1
}
...
method _do_clone {} {
	...
	${NS}::label $args.origin_l -text [mc "源路徑："]
	...
	${NS}::label $args.where_l -text [mc "目標目錄："]
...
method _open_origin {} {
	...
	if {![_is_git [file join $p .git]] && ![_is_git $p]} {
		error_popup [mc "不是 Git 倉庫: %s" [file tail $p]]
		return
	}
	set origin_url $p
}
...
method _do_clone2 {} {
	...
	if {$clone_type eq {hardlink} && ![file isdirectory $origin_url]} {
		error_popup [mc "標準類型僅適用於本地倉庫。"]
		return
	}
	if {$clone_type eq {shared} && ![file isdirectory $origin_url]} {
		error_popup [mc "共享類型僅適用於本地倉庫。"]
		return
	}
	...
	if {$clone_type eq {hardlink} || $clone_type eq {shared}} {
		...
		if {$objdir eq {}} {
			error_popup [mc "不是 Git 倉庫: %s" [file tail $origin_url]]
			return
		}
	}
	...
	if {[file exists $local_path]} {
		error_popup [mc " %s 路徑已存在。" $local_path]
		return
	}
	...
	if {[catch {
		...
		error_popup [strcat [mc "組態遠端倉庫失敗"] "\n\n$err"]
		return
	}
	...
	hardlink {
		...
		if {[file exists [file join $objdir info alternates]]} {
			...
			if {[catch {
				...
				_clone_failed $this [mc "無法複製 objects/info/alternates: %s" $err]
				$status_op stop
				return
			}
		}
	...
	if {$clone_type eq {hardlink} || $clone_type eq {shared}} {
		...
		if {[catch {
			...
			_clone_failed $this [mc "不是 Git 倉庫: %s" [file tail $origin_url]]
			return 0
		}
		cd $pwd
...
method _copy_files {objdir tocopy} {
	...
	foreach p $tocopy {
		if {[catch {
			...
			_clone_failed $this [mc "無法複製目標： %s" $err]
			$status_op stop
			return 0
		}
	}
...
method _link_files {objdir tolink} {
	...
	for {set i 0} {$i < $total} {} {
		...
		if {[catch {
			...
			_clone_failed $this [mc "無法建立對象永久連結: %s" $err]
			$status_op stop
			return 0
		}
...
method _do_clone_tags {ok} {
	if {$ok} {
		...
		$o_cons done $ok
		_clone_failed $this [mc "無法獲取分支和對象。詳情查看控制台輸出。"]
	}
}
...
method _do_clone_HEAD {ok} {
	if {$ok} {
		...
		$o_cons done $ok
		_clone_failed $this [mc "無法獲取標籤。詳情查看控制台輸出。"]
	}
}
...
method _do_clone_full_end {ok} {
	...
	if {$ok} {
		...
		_clone_failed $this [mc "無法確定 HEAD。詳情查看控制台輸出。"]
	}
}
...
method _clone_failed {{why {}}} {
	if {[catch {file delete -force $local_path} err]} {
		set why [strcat \
			$why \
			"\n\n" \
			[mc "無法清理 %s" $local_path] \
			"\n\n" \
			$err]
	}
	if {$why ne {}} {
		update
		error_popup [strcat [mc "克隆失敗。"] "\n" $why]
	}
}
...
method _do_open {} {
	...
	${NS}::frame $w_body.where
	${NS}::label $w_body.where.l -text [mc "倉庫："]
	${NS}::entry $w_body.where.t \
		-textvariable @local_path \
		-width 50
	${NS}::button $w_body.where.b \
...
method _do_open2 {} {
	if {![_is_git [file join $local_path .git] actualgit]} {
		error_popup [mc "不是 Git 倉庫: %s" [file tail $local_path]]
		return
	}

	if {[catch {cd $local_path} err]} {
		error_popup [strcat \
			[mc "開啟倉庫失敗：%s" $local_path] \
			"\n\n$err"]
		return
	}
