...
版權所有 c 2006-2010 Shawn Pearce 及其他貢獻者。

本程序是自由軟件；您可以遵照自由軟件基金會發佈的 GNU 通用公共許可證條款，
重新發佈和/或修改本程序。許可證版本為第 2 版，或者（根據您的選擇）任何更新的版本。

本程序的發佈旨在希望它能有用，但不提供任何擔保；甚至沒有隱含的適銷性或特定用途適用性的擔保。
詳情請參閱 GNU 通用公共許可證。

您應該已經隨本程序收到了一份 GNU 通用公共許可證的副本；
如果沒有，請查閱 <https://www.gnu.org/licenses/>。}]
...
proc apply_config {} {
	...
	foreach option $font_descs {
		...
		if {[catch {
			...
			error_popup [strcat [mc "在 %s 中指定的字型無效：" "gui.$name"] "\n\n$err"]
		}
...
set font_descs {
	{fontui   font_ui   {mc "主字型"}}
	{fontdiff font_diff {mc "差異對比 / 控制台字型"}}
}
...
if {$_git eq {}} {
	...
	tk_messageBox \
		-icon error \
		-type ok \
		-title [mc "git-gui: 嚴重錯誤"] \
		-message [mc "PATH設定中沒有git。"]
	exit 1
}
...
if {![regsub {^git version } $_git_version {} _git_version]} {
	...
	tk_messageBox \
		...
		-type ok \
		-title [mc "git-gui: 嚴重錯誤"] \
		-message [strcat [mc "無法解析 Git 版本字符串："] "\n\n$_git_version"]
	exit 1
}
...
if {![regexp {^[1-9]+(\.[0-9]+)+$} $_git_version]} {
	...
	if {[tk_messageBox \
		...
		-message [mc "無法確定 Git 版本。

%s 宣稱自身版本是 '%s'.

%s 需要至少 Git 1.5.0 後之後版本。
...
if {[git-version < 1.5]} {
	...
	tk_messageBox \
		...
		-type ok \
		-title [mc "git-gui: 嚴重錯誤"] \
		-message "[appname] 需要 Git 1.5.0 或之後版本。
...
if {![file isdirectory $_gitdir]} {
	...
	error_popup [strcat [mc "找不到Git 目錄："] "\n\n$_gitdir"]
	exit 1
}
...
if {$_prefix ne {}} {
	...
	if {[catch {cd $cdup} err]} {
		...
		error_popup [strcat [mc "無法移動到工作目錄頂端："] "\n\n$err"]
		exit 1
	}
	...
	if {[is_bare]} {
		...
		error_popup [strcat [mc "無法使用裸存儲庫："] "\n\n$_gitdir"]
		exit 1
	}
	if {$_gitworktree eq {}} {
	...
	if {[catch {cd $_gitworktree} err]} {
		...
		error_popup [strcat [mc "沒有工作目錄"] " $_gitworktree:\n\n$err"]
		exit 1
	}
	set _gitworktree [pwd]
}
...
proc run_prepare_commit_msg_hook {} {
	...
	ui_status [mc "正在調用 prepare-commit-msg 鉤子..."]
	set pch_error {}
...
proc prepare_commit_msg_hook_wait {fd_ph} {
	...
	if {[eof $fd_ph]} {
		if {[catch {close $fd_ph}]} {
			ui_status [mc "提交已被 prepare-commit-msg 鉤子拒絕。"]
...
proc mapicon {w state path} {
	...
	if {[catch {set r $all_icons($state$w)}]} {
		puts "錯誤: 沒有為 $w 狀態={$state} $path 定義圖示"
		return file_plain
	}
	return $r
}
	...
	foreach path $to_display {
		...
		if {$displayed > $display_limit && [string index $m 1] eq {O} } {
			if {!$files_warning} {
				...
				set files_warning 1
				info_popup [mc "已達顯示限制 (gui.maxfilesdisplayed = %s) ，未顯示全部 %s 個檔案。" \
...
foreach i {
		...
		{T {mc "檔案類型已變更，未暫存"}}
		{MT {mc "檔案類型已變更，舊類型已暫存待提交"}}
		{AT {mc "檔案類型已變更，舊類型已暫存待提交"}}
		{T {mc "檔案類型已變更，已暫存"}}
		{TM {mc "檔案類型變更已暫存，修改未暫存"}}
		{TD {mc "檔案類型變更已暫存，檔案遺失"}}
...
proc do_git_gui {} {
	...
	if {$exe eq {}} {
		error_popup [mc "PATH設定中沒有Git GUI"]
	} else {
		global env
		global _gitdir _gitworktree
...
proc toggle_or_diff {mode w args} {
	...
	if {$col == 0 && $y > 1} {
		...
		if {$w eq $ui_index} {
			update_indexinfo \
				"從提交中取消暫存 [short_path $path]" \
				[list $path] \
				[concat $after {ui_ready;}]
		} elseif {$w eq $ui_workdir} {
			update_index \
				"將 [short_path $path] 新增至暫存區" \
				[list $path] \
				[concat $after {ui_ready;}]
		}
	} else {
...
.mbar add cascade -label [mc 儲存庫] -menu .mbar.repository
.mbar add cascade -label [mc 編輯] -menu .mbar.edit
if {[is_enabled branch]} {
	.mbar add cascade -label [mc 分支] -menu .mbar.branch
}
if {[is_enabled multicommit] || [is_enabled singlecommit]} {
	.mbar add cascade -label [mc 提交@@noun] -menu .mbar.commit
}
if {[is_enabled transport]} {
	.mbar add cascade -label [mc 合併] -menu .mbar.merge
	.mbar add cascade -label [mc 遠端] -menu .mbar.remote
}
if {[is_enabled multicommit] || [is_enabled singlecommit]} {
	.mbar add cascade -label [mc 工具] -menu .mbar.tools
}
...
.mbar.repository add command \
	-label [mc "視覺化當前分支的歷史記錄"] \
	-command {do_gitk $current_branch}
set ui_visualize_current [.mbar.repository index last]
.mbar.repository add command \
	-label [mc "視覺化所有分支歷史記錄"] \
	-command {do_gitk --all}
.mbar.repository add separator
...
proc current_branch_write {args} {
	...
	.mbar.repository entryconf $::ui_browse_current \
		-label [mc "瀏覽 %s 的檔案" $current_branch]
	.mbar.repository entryconf $::ui_visualize_current \
		-label [mc "視覺化 %s 的歷史記錄" $current_branch]
}
trace add variable current_branch write current_branch_write

if {[is_enabled multicommit]} {
	.mbar.repository add command -label [mc "資料庫統計資料"] \
		-command do_stats

	.mbar.repository add command -label [mc "壓縮資料庫"] \
		-command do_gc

	.mbar.repository add command -label [mc "驗證資料庫"] \
...
if {[is_MacOSX]} {
	...
	.mbar.repository add command -label [mc 退出] \
		-command do_quit \
		-accelerator $M1T-Q
}
...
#
menu .mbar.edit
.mbar.edit add command -label [mc 復原] \
	-command {catch {[focus] edit undo}} \
	-accelerator $M1T-Z
.mbar.edit add command -label [mc 重做] \
	-command {catch {[focus] edit redo}} \
	-accelerator $M1T-Y
.mbar.edit add separator
.mbar.edit add command -label [mc 剪下] \
	-command {catch {tk_textCut [focus]}} \
	-accelerator $M1T-X
.mbar.edit add command -label [mc 複製] \
	-command {catch {tk_textCopy [focus]}} \
	-accelerator $M1T-C
.mbar.edit add command -label [mc 貼上] \
	-command {catch {tk_textPaste [focus]; [focus] see insert}} \
	-accelerator $M1T-V
.mbar.edit add command -label [mc 刪除] \
	-command {catch {[focus] delete sel.first sel.last}} \
	-accelerator Del
.mbar.edit add separator
.mbar.edit add command -label [mc "全選"] \
...
if {[is_enabled branch]} {
	...
	.mbar.branch add command -label [mc "建立..."] \
		-command branch_create::dialog \
		-accelerator $M1T-N
	lappend disable_on_lock [list .mbar.branch entryconf \
		[.mbar.branch index last] -state]

	.mbar.branch add command -label [mc "簽出..."] \
		-command branch_checkout::dialog \
		-accelerator $M1T-O
	lappend disable_on_lock [list .mbar.branch entryconf \
		[.mbar.branch index last] -state]

	.mbar.branch add command -label [mc "重命名..."] \
		-command branch_rename::dialog
	lappend disable_on_lock [list .mbar.branch entryconf \
		[.mbar.branch index last] -state]

	.mbar.branch add command -label [mc "刪除..."] \
		-command branch_delete::dialog
	lappend disable_on_lock [list .mbar.branch entryconf \
		[.mbar.branch index last] -state]

	.mbar.branch add command -label [mc "重置..."] \
...
if {[is_enabled multicommit] || [is_enabled singlecommit]} {
	...
	.mbar.commit add command -label [mc 重新掃描] \
		-command ui_do_rescan \
		-accelerator F5
	lappend disable_on_lock \
		[list .mbar.commit entryconf [.mbar.commit index last] -state]

	.mbar.commit add command -label [mc "暫存至提交"] \
		-command do_add_selection \
		-accelerator $M1T-T
	lappend disable_on_lock \
		[list .mbar.commit entryconf [.mbar.commit index last] -state]

	.mbar.commit add command -label [mc "將已修改的檔案暫存至提交"] \
		-command do_add_all \
		-accelerator $M1T-I
	lappend disable_on_lock \
		[list .mbar.commit entryconf [.mbar.commit index last] -state]

	.mbar.commit add command -label [mc "從提交中取消暫存"] \
		-command do_unstage_selection \
		-accelerator $M1T-U
	lappend disable_on_lock \
		[list .mbar.commit entryconf [.mbar.commit index last] -state]

	.mbar.commit add command -label [mc "還原變更"] \
	...
	.mbar.commit add command -label [mc "顯示較少上下文"] \
		-command show_less_context \
		-accelerator $M1T-\-

	.mbar.commit add command -label [mc "顯示更多上下文"] \
	...
	if {![is_enabled nocommitmsg]} {
		.mbar.commit add command -label [mc "簽署"] \
			-command do_signoff \
			-accelerator $M1T-S
	}
...
if {[is_enabled branch]} {
	menu .mbar.merge
	.mbar.merge add command -label [mc "本地合併..."] \
		-command merge::dialog \
		-accelerator $M1T-M
	lappend disable_on_lock \
		[list .mbar.merge entryconf [.mbar.merge index last] -state]
	.mbar.merge add command -label [mc "中止合併..."] \
		-command merge::reset_hard
	lappend disable_on_lock \
		[list .mbar.merge entryconf [.mbar.merge index last] -state]
}
...
if {[is_MacOSX]} {
	...
	#
	.mbar.edit add separator
	.mbar.edit add command -label [mc "選項..."] \
		-command do_options
}
...
if {[is_enabled multicommit] || [is_enabled singlecommit]} {
	...
	$tools_menubar add command -label [mc "新增..."] -command tools_add::dialog
	$tools_menubar add command -label [mc "移除..."] -command tools_remove::dialog
	set tools_tailcnt 3
...
# -- Help Menu
#
.mbar add cascade -label [mc 說明] -menu .mbar.help
menu .mbar.help

if {[is_MacOSX]} {
	.mbar.apple add command -label [mc "關於 %s" [appname]] \
		-command do_about
	.mbar.apple add separator
} else {
	.mbar.help add command -label [mc "關於 %s" [appname]] \
		-command do_about
}
. configure -menu .mbar
...
.mbar.help add command -label [mc "線上文件"] \
	-command [list start_browser $doc_url]

.mbar.help add command -label [mc "顯示 SSH 金鑰"] \
	-command do_ssh_key
...
set subcommand_args {}
proc usage {} {
	set s "[mc 用法:] $::argv0 $::subcommand $::subcommand_args"
	if {[tk windowingsystem] eq "win32"} {
		wm withdraw .
	...
	if {$head eq {}} {
		...
		if {[regexp {^[0-9a-f]{1,39}$} $head]} {
			if {[catch {
				...
				if {[tk windowingsystem] eq "win32"} {
					tk_messageBox -icon error -title [mc 錯誤] -message $err
				} else {
					puts stderr $err
				}
				exit 1
	...
	blame   {
		if {$head eq {} && ![file exists $path]} {
			...
			tk_messageBox \
				...
				-type ok \
				-title [mc "git-gui: 嚴重錯誤"] \
				-message [mc "嚴重：無法統計路徑 %s：沒有此檔案或目錄" $path]
			exit 1
		}
...
tlabel .vpane.files.workdir.title -text [mc "未暫存的變更"] \
...
${NS}::button .vpane.lower.commarea.buttons.rescan -text [mc 重新掃描] \
	-command ui_do_rescan
pack .vpane.lower.commarea.buttons.rescan -side top -fill x
lappend disable_on_lock \
	{.vpane.lower.commarea.buttons.rescan conf -state}

${NS}::button .vpane.lower.commarea.buttons.incall -text [mc "暫存已修改"] \
	-command do_add_all
...
if {![is_enabled nocommitmsg]} {
	${NS}::button .vpane.lower.commarea.buttons.signoff -text [mc "簽署"] \
		-command do_signoff
...
if {![is_enabled nocommit]} {
	${NS}::button .vpane.lower.commarea.buttons.push -text [mc 推送] \
		-command do_push_anywhere
...
proc trace_commit_type {varname args} {
	...
	initial       {set txt [mc "初始提交訊息："]}
	amend         {set txt [mc "修正後的提交訊息："]}
	amend-initial {set txt [mc "修正後的初始提交訊息："]}
	amend-merge   {set txt [mc "修正後的合併提交訊息："]}
	merge         {set txt [mc "合併提交訊息："]}
	*             {set txt [mc "提交訊息："]}
	}
